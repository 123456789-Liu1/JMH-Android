...
| **网络使用** | 仅用于检查更新，不上传任何数据 |

> **关于网络权限**：应用声明了 `INTERNET` 权限，唯一用途是请求仓库中的版本信息文件
> 并在你确认后下载新版 APK。加密、解密、预览等全部核心功能**完全离线**，
> 不存在任何形式的数据上传、遥测或云端同步。
...
除了保护自己的隐私，JMH 也可以用来做**点对点的加密通信**：

- **导出发送**：在文件列表里选择「导出为加密文件（分享）」，
  为它设置一个**独立的分享密码**（强制 12 位复杂密码），生成的 `.jmh` 文件可以通过任何渠道发给对方
- **导入接收**：对方在主页点击「导入加密文件」，选择文件并输入分享密码，
  文件就会进入他自己的金库，之后用他自己的主密码即可访问
- 分享密码与本机主密码**完全独立**，泄露分享密码不会影响你的金库安全
- 可勾选「设为默认导出文件夹」，下次导出自动定位

> 分享文件使用独立的随机盐与密钥，因此**同一个密码在不同文件上会产生完全不同的密钥**。
> 若密码泄露，只需重新导出一份不同密码的文件，旧文件的安全性与其余文件无关。

### 检查更新

- **自动检查**：启动时后台静默检查，只有发现新版本才会提示；
  **连不上网络时不会有任何打扰**，应用照常使用
- **手动检查**：设置页「检查更新」，三种结果都有明确反馈（连不上可重试 / 已是最新 / 发现新版本）
- **应用内更新**：下载后直接调起系统安装器，**覆盖安装会保留全部数据与设置**
...
**Q：应用会联网吗？会不会上传我的文件？**
不会。应用仅在**检查更新**时请求仓库中的一个静态版本信息文件，
并在你确认后下载新版 APK。加密、解密、预览全程离线，
不存在任何形式的数据上传、埋点或云端同步。

**Q：分享文件被别人拿到，会被暴力破解吗？**
分享密码强制要求 12 位且包含大小写字母、数字与特殊符号，
配合 15 万次迭代的 PBKDF2，离线爆破成本极高。
此外应用会在连续输错后递增重试延迟。但仍请通过可信渠道传递密码。

**Q：更新会不会清空我的加密文件？**
不会。覆盖安装会完整保留应用数据与金库文件。前提是应用签名一致——
本项目已固定签名，从 v1.0.1 起的所有版本均可直接覆盖升级。
